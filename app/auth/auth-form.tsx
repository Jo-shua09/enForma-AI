...
export function AuthForm() {
  ...
  useEffect(() => {
    if (!busy && ready && user) {
      // If the plan is undefined, null, or pending, force them to the pricing step
      if (!user.current_plan || user.current_plan === "pending") {
        if (step !== "pricing") {
          setStep("pricing");
        }
      } else {
        // Only valid active users get routed to the dashboard automatically
        if (step === "details") {
          router.push("/dashboard");
        }
      }
    }
  }, [busy, ready, user, step, router]);
